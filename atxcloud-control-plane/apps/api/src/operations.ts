...
export async function recordOperation(type:string,actorId:string,targetType:string,targetId:string,metadata:any={}){
...
export async function softDeleteServer(publicId:string, actorId:string, staff=false){
  return tx(async c=>{
    ...
    try{await ptero.deleteServer(Number(r.rows[0].ptero_server_id));}catch(e){throw Object.assign(new Error('pterodactyl_delete_failed'),{statusCode:502,cause:e});}
    await c.query('UPDATE servers SET deleted_at=now(),updated_at=now() WHERE id=$1',[r.rows[0].id]);
    return r.rows[0];
  });
}

export async function setServerSuspended(publicId:string,suspended:boolean,actorId:string,staff=true){
  return tx(async c=>{
    const r=await c.query<any>('SELECT * FROM servers WHERE public_id=$1 AND deleted_at IS NULL FOR UPDATE',[publicId]);
    if(!r.rows[0])throw Object.assign(new Error('server_not_found'),{statusCode:404});
    if(!staff && r.rows[0].owner_id!==actorId)throw Object.assign(new Error('staff_required'),{statusCode:403});
    if(suspended)await ptero.suspendServer(Number(r.rows[0].ptero_server_id));else await ptero.unsuspendServer(Number(r.rows[0].ptero_server_id));
