...
async function pages<T>(path:string, params:Record<string,unknown>={}):Promise<T[]> {
  const out:T[]=[];
  for(let page=1;;page++){
    const r=await app().get(path,{params:{...params,page,per_page:100}}) as {data:PteroPage<T>};
    out.push(...(r.data.data || []));
    const total=r.data.meta?.pagination?.total_pages;
    if(!total || page>=total) return out;
  }
}
...
export function pteroError(error:unknown){
  const e:any=error;
  const status=e?.response?.status;
  const detail=e?.response?.data?.errors?.[0]?.detail || e?.message || 'Pterodactyl request failed';
  return Object.assign(new Error(detail),{statusCode:status && status>=400 && status<600 ? status : 502, pteroStatus:status, pteroData:e?.response?.data});
}
