...
export async function transferCoins(fromUserId:number,toUserId:number,amount:number,idempotencyKey:string) {
  ...
  if (!/^[A-Za-z0-9._:-]{8,160}$/.test(idempotencyKey)) throw Object.assign(new Error('invalid_idempotency_key'), { statusCode:400 });
  return tx(async (c:PoolClient) => {
    ...
    const users=await c.query<any>('SELECT id,coins,blacklisted FROM users WHERE id=ANY($1::int[]) FOR UPDATE',[ids]);
    ...
    await c.query('INSERT INTO coin_ledger(idempotency_key,user_id,actor_id,amount,type,reference) VALUES($1,$2,$3,$4,$5,$6)',[idempotencyKey,fromUserId,fromUserId,-amount,'gift',String(toUserId)]);
    await c.query('INSERT INTO coin_ledger(idempotency_key,user_id,actor_id,amount,type,reference) VALUES($1,$2,$3,$4,$5,$6)',[`${idempotencyKey}:credit`,toUserId,fromUserId,amount,'gift',String(fromUserId)]);
    return {idempotent:false};
  });
}

export async function adjustCoins(actorId:number,targetId:number,amount:number,reason:string,idempotencyKey:string){
  return tx(async c=>{
